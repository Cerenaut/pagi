...
class SemanticEmbedding(Embedding):
  """
  Embedding transforms a large number of disjoint classes (e.g. words) into a dense or sparse combination of
  fewer dimensions. Good embeddings have other features represented in the relationships between classes.
  """

  def create(self, corpus_files, model_file, shape=None, sparsity=0, eos='<end>'):  # pylint: disable=arguments-differ
    del sparsity
    ...
    #sg = 0 # CBOW
    sg = 1 # skip-gram
    model = gensim.models.Word2Vec(data, size=size, window=window_size, min_count=min_count, workers=workers,
                                   iter=epochs, sg=sg)
  ...
  #       self._key_index.update({key:row_index})
  #       self._index_key.append(key)#[row_index] = key
